...
Once a dataset has been tidied, often one of the first things we want to do is generate summary statistics. In the `mpg` dataset that is built into the `Tidyverse`, how would be go about generating (e.g.) the means and standard deviations grouped by car manufacturer for one of our variables? Have a look at the following video where I walk you through this worksheet. Then I want you to work through the content by writing (and running) the script on your own machine.
...
First well work out the average highway fuel economy (measured in the `hwy` column) for each car manufacturer. Then we'll  work out how many of each manufacturer there are using the `n()` function.
...
Wouldn't it be great if we could also visualize the entire dataset? We can use the function `vis_dat()` from the packages `visdat::` to do just that.

```{r}
library(visdat)
vis_dat(mpg_with_mean)
```

Note, our variables (i.e., the columns in our dataset) are vectors (sequences of data elements) that include type `character`, `integer`, or `numeric`. Vectors of type `integer` are whole numbers. You can think of integer vectors as being a subset of numeric vectors.
